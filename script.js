(() => {
  ...
  const revealTargets = document.querySelectorAll(
    '.about__story, .about__timeline li, .spec-sheet, .schematic-wrap, .service-card, .skills__group, .process__step, .project-card, .certs__row, .testimonial-card, .faq__item, .contact__grid > *, .calendly-block'
  );
  ...
  form.addEventListener('submit', (e) => {
    ...
    const messages = {
      fr: {
        invalid: 'Merci de renseigner tous les champs.',
        ready: 'Message prêt — pensez à connecter ce formulaire à un service d\u2019envoi (Formspree, EmailJS, etc.) ou contactez directement via WhatsApp.'
      },
      en: {
        invalid: 'Please fill in all fields.',
        ready: 'Message ready — remember to connect this form to a sending service (Formspree, EmailJS, etc.) or reach out directly via WhatsApp.'
      }
    };
    const m = messages[currentLang] || messages.fr;
    if (!form.checkValidity()) {
      formNote.textContent = m.invalid;
      return;
    }
    formNote.textContent = m.ready;
    form.reset();
  });
